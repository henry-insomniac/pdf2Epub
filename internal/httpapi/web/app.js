...
const stages={waiting:["等待中","任务已建立，正在准备解析器。"],preflight:["预检","正在检查页数、安全限制与适合版式。"],extracting:["提取内容","正在读取文字与插图，或渲染固定版式页面。"],rebuilding:["重建结构","正在组织段落、目录或双页顺序。"],packaging:["生成 EPUB","正在写入章节、页面、封面与样式。"],validating:["规范校验","正在运行 EPUBCheck。"],completed:["转换完成","EPUB 已通过校验，可以下载。"],failed:["转换失败","转换没有生成可下载文件。"],canceled:["已取消","任务已取消，临时文件已清理。"]};
...
$("uploadForm").addEventListener("submit",async event=>{event.preventDefault();if(!fileInput.files[0])return;const form=new FormData();form.append("file",fileInput.files[0]);form.append("mode",document.querySelector('input[name="mode"]:checked')?.value||"auto");$("submitButton").disabled=true;try{const job=await api("/api/v1/jobs",{method:"POST",body:form});state.jobId=job.id;sessionStorage.setItem("btc_job_id",job.id);renderJob(job);pollJob()}catch(error){showMessage(error.message);$("submitButton").disabled=false}});
